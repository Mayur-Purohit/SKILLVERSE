...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function refreshImportantCard() {
        const container = document.getElementById('important-card-content');
        if (!container) return;
        ...
        fetch('/api/important')
            ...
            .then(data => {
                ...
                <div class="important-item"
                    ...
                    <div
                        ...
                        Next Event</div>
                    <div class="important-title" style="color: #fff;">${data.event.title}</div>
                    <div class="important-meta" style="color: #fbbf24;">${data.event.date} • ${data.event.time || ''}</div>
                </div>
            `;
                } else if (data.todo) {
                    const isHigh = data.todo.priority === 'high';
                    const todoColor = isHigh ? '#ef4444' : '#3b82f6';
                    const bg = isHigh ? '' : 'background: rgba(59, 130, 246, 0.05);';
                    html = `
                <div class="important-item"
                    style="border-left-color: ${todoColor}; ${bg}">
                    <div
                        style="font-size: 0.65rem; color: ${todoColor}; font-weight: 800; margin-bottom: 4px; text-transform: uppercase;">
                        ${data.todo.label}</div>
                    <div class="important-title" style="color: #fff;">${data.todo.title}</div>
                    <div class="important-meta" style="${!isHigh ? 'color: ' + todoColor : ''}">Due ${data.todo.due_date}
                    </div>
                </div>
            `;
                } else {
                    html = `
                <div class="important-item">
                    <div class="important-title" style="color: #fff;">All Clear!</div>
                    <div class="important-meta">No urgent events or high priority tasks.</div>
                </div>
            `;
                }

                // Simple check to avoid unnecessary reflow if content is identical
            ...
            .catch(e => console.error("Failed to refresh important card", e));
    }

    window.refreshImportantCard = refreshImportantCard;
    ...
    setInterval(() => {
        ...
        // Refresh important card and active tasks every 10 seconds
        if (seconds % 10 === 0) {
            refreshImportantCard();
            if (window.refreshActiveTasksList) {
                window.refreshActiveTasksList();
            }
        }
    }, 1000);
